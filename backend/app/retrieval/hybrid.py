...
class HybridRetriever:
    ...
    async def recall_all(self, question: str, keywords: list[str] | None = None) -> HybridRecallResult:
        """执行三路召回并返回合并结果。"""
        kws = keywords if keywords is not None else extract_keywords(question)
        columns, recall_mode = await self.recall_columns_only(question, kws)
        metrics = await self.recall_metrics_only(question, kws)
        values = await self.recall_field_values_only(question, kws)

        modes = {c.recall_mode for c in columns} | {m.recall_mode for m in metrics} | {v.recall_mode for v in values}
        ...
        *,
        ...
        if use_es:
            try:
                ...
                if vectors:
                    hits = await self._es.search_vector("column", vectors[0], top_k=top_k)
                    if hits:
                        return [
                            RecalledColumn(
        ...
        return rank_columns_by_keywords(rows, keywords, top_k=top_k)
